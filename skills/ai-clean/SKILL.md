...
4. **Pre-output gate (run BEFORE the full self-check; these fail most often).**
   ...
   - [ ] **Unfilled placeholders.** Search for bracketed fill-in-the-blanks left unreplaced ("[Entertainer's Name]", "(Add your channel URL here)"). Any hit? Either fill it with the real detail if you have it, or rewrite the sentence so it doesn't need one — never leave the blank in.
